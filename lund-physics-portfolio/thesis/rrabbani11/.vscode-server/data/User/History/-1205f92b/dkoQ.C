...
void analyze(std::vector<std::string> filenames = {"../pythia.root"}) {
  ...
  void plotProfiles() {
    // Open the ROOT file
    TFile* f = new TFile("yields.root", "READ");
    if (!f->IsOpen()) {
      std::cerr << "Error: Could not open file!" << std::endl;
      return;
    }
  
    // Retrieve the TProfile histograms
    TProfile* hPiCent = f->Get<TProfile>("hPiCent");
    TProfile* hPCent = f->Get<TProfile>("hPCent");
    TProfile* hKCent = f->Get<TProfile>("hKCent");
    TProfile* hLCent = f->Get<TProfile>("hLCent");
    TProfile* hXCent = f->Get<TProfile>("hXCent");
    TProfile* hOmegaCent = f->Get<TProfile>("hOmegaCent");
  
    // Check if histograms were retrieved successfully
    if (!hPiCent || !hPCent || !hKCent || !hLCent || !hXCent || !hOmegaCent) {
      std::cerr << "Error: Could not load one or more histograms!" << std::endl;
      return;
    }
    ...
    hPiCent->Draw();
    hPiCent->SetLineColor(kRed); // Set line color for hPiCent
    hPiCent->SetMarkerStyle(20); // Set marker style for hPiCent
  
    // Draw the remaining histograms on the same canvas
    hPCent->Draw("same");
    hPCent->SetLineColor(kBlue); // Set line color for hPCent
    hPCent->SetMarkerStyle(21);  // Set marker style for hPCent
  
    hKCent->Draw("same");
    hKCent->SetLineColor(kGreen); // Set line color for hKCent
    hKCent->SetMarkerStyle(22);   // Set marker style for hKCent
  
    hLCent->Draw("same");
    hLCent->SetLineColor(kMagenta); // Set line color for hLCent
    hLCent->SetMarkerStyle(23);     // Set marker style for hLCent
  
    hXCent->Draw("same");
    hXCent->SetLineColor(kCyan); // Set line color for hXCent
    hXCent->SetMarkerStyle(24);  // Set marker style for hXCent
  
    hOmegaCent->Draw("same");
    hOmegaCent->SetLineColor(kOrange); // Set line color for hOmegaCent
    hOmegaCent->SetMarkerStyle(25);    // Set marker style for hOmegaCent
  
    // Add a legend
    TLegend* legend = new TLegend(0.7, 0.7, 0.9, 0.9);
    legend->AddEntry(hPiCent, "Pions", "lp");
    legend->AddEntry(hPCent, "Protons", "lp");
    legend->AddEntry(hKCent, "Kaons", "lp");
    legend->AddEntry(hLCent, "Lambdas", "lp");
    legend->AddEntry(hXCent, "Xis", "lp");
    legend->AddEntry(hOmegaCent, "Omegas", "lp");
    legend->Draw();
    hPiCent->SetXTitle("Centrality");
    hPiCent->SetYTitle("Yield");
  
    // Save the canvas as an image (optional)
    c1->SaveAs("profiles.png");
