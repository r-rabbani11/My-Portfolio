...
constexpr double beta_avg[nMultClasses]       = {0.488, 0.44,  0.4,   0.378, 0.325, 0.287, 0.25,  0.20,  0.10};
constexpr double T_kin[nMultClasses]          = {0.163, 0.174, 0.180, 0.181, 0.184, 0.184, 0.183, 0.181, 0.173};
constexpr double n[nMultClasses]              = {1.47,  1.70,  2.01,  2.25,  2.89,  3.48,  4.2,   5.71,  8.0};
    ...
    for (Int_t kMultClass = 0; kMultClass < nMultClasses; kMultClass++) {
        ...
        try {
            ...
            if (kMultClass == nMultClasses - 1) {
                params.T = std::max(params.T, 0.165);
                ...
                while (retries < MAX_RETRIES && !success) {
                    try {
                        ...
                        if (retries >= MAX_RETRIES) {
                            ...
                            // Recreate generator instead of Reset()
