...
void GenFistPP(Int_t nEventsPerPercent=100, const double kCorrVolume = 3., const int kSeed = 0){
  ...
  RandomGenerators::SetSeed(31415);
  ...
  TFile *out_file = new TFile(Form("/home/rrabbani11/fist_data/trial.root", gRandom->GetSeed()),"recreate");
  out_file->cd();
  TTree* treeOut = 0;
  ...
  for(Int_t kMultClass = 8; kMultClass < nMultClasses; kMultClass++) {
