...
//constexpr int kNumberOfEvents = 10000;
...
void GenFistPP(Int_t nEventsPerPercent=1000, const double kCorrVolume = 3., const int kSeed = 0){
  ...
  TFile *out_file = new TFile(Form("/home/rrabbani11/fist_data/trial%d.root", gRandom->GetSeed()),"recreate");
  out_file->cd();
  TTree* treeOut = 0;
  ...
  //for(Int_t kMultClass = 7; kMultClass < nMultClasses; kMultClass++) {
    ...
    std::cout << kNumberOfEvents << std::endl;
    for (int i = 0; i < kNumberOfEvents; ++i){

      nTotalEvents++;
      if (i%500 == 0) std::cout << "generated " << i << "/" << kNumberOfEvents << "events in this class..." << std::endl;
      if (i == 507) continue;
	...
	track->fIsPrimary = 1;
      ...
      std::cout << "end single event " << i << std::endl;
