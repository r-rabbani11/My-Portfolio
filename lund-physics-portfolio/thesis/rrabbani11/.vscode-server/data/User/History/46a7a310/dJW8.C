...
constexpr double beta_avg[nMultClasses]       = {0.488, 0.44,  0.4,   0.378, 0.325, 0.287, 0.25,  0.20,  0.10}; // Reduced last value
constexpr double T_kin[nMultClasses]          = {0.163, 0.174, 0.180, 0.181, 0.184, 0.184, 0.183, 0.181, 0.173};
constexpr double n[nMultClasses]              = {1.47,  1.70,  2.01,  2.25,  2.89,  3.48,  4.2,   5.71,  8.0};  // Reduced last value
...
void GenFistPP(Int_t nEventsPerPercent=100, const double kCorrVolume = 3., const int kSeed = 0) {
    ...
    for (Int_t kMultClass = 0; kMultClass < nMultClasses; kMultClass++) {
        ...
        try {
            ...
            // Special handling for last class
            if (kMultClass == nMultClasses - 1) {
                params.T = std::max(params.T, 0.165);  // Ensure minimum temperature
                std::cout << "Applying stability fixes for last multiplicity class..." << std::endl;
            }
            ...
            // Apply safety limits
            betaS = std::min(betaS, 0.7);
            double actual_n = std::min(n[kMultClass], 10.0);
                ...
                while (retries < MAX_RETRIES && !success) {
                    try {
                        ...
                        if (retries >= MAX_RETRIES) {
                            std::cerr << "Skipping event after " << MAX_RETRIES << " retries in class " 
                                      << kMultClass << ": " << e.what() << std::endl;
                        } else {
