...
// Print bin values and error bars
  ...
  leg->AddEntry(k_sim, "2K^{0}_{S}/(#pi^{+} + #pi^{-}) #times2", "ep");
  leg->AddEntry(L_sim, "#Lambda + #bar{#Lambda}/(#pi^{+} + #pi^{-})  #times2", "ep");
  leg->AddEntry(X_sim, "#Xi^{-} + #bar{#Xi}^{+}/(#pi^{+} + #pi^{-})  #times6", "ep");
  leg->AddEntry(O_sim, "#Omega^{-} + #bar{#Omega}^{+}/(#pi^{+} + #pi^{-}) #times16", "ep");
  leg->AddEntry((TObject*)0, "FIST:", "");
  leg->AddEntry(k_fist, "K^{0}/(#pi^{+} + #pi^{-})", "ep");
  leg->AddEntry(L_fist, "#Lambda + #bar{#Lambda}/(#pi^{+} + #pi^{-}) #times2", "ep");
  leg->AddEntry(X_fist, "#Xi^{-} + #bar{#Xi}^{+}/(#pi^{+} + #pi^{-}) #times6", "ep");
  leg->AddEntry(O_fist, "#Omega^{-} + #bar{#Omega}^{+}/(#pi^{+} + #pi^{-}) #times16", "ep");
  leg->AddEntry((TObject*)0, "ALICE:", "");
  leg->AddEntry(k_exp, "2K^{0}_{S}/(#pi^{+} + #pi^{-}) #times2", "p");
  leg->AddEntry(L_exp, "#Lambda + #bar{#Lambda}/(#pi^{+} + #pi^{-}) #times2", "p");
  leg->AddEntry(X_exp, "#Xi^{-} + #bar{#Xi}^{+}/(#pi^{+} + #pi^{-}) #times6", "p");
  leg->AddEntry(O_exp, "#Omega^{-} + #bar{#Omega}^{+}/(#pi^{+} + #pi^{-}) #times16", "p");
  leg->Draw();
...
void drawRatiosXiXibar() {
    ...
    // Open the ROOT file
    TFile* yieldsFile = new TFile("k2pi_xixibar_yieldskminus.root", "READ");
    if (!yieldsFile || yieldsFile->IsZombie()) {
...
for (int i = 1; i <= hPiMinus->GetNbinsX(); ++i)
    ...
    hKPlus->SetTitle("K/#pi ratio within Xi-#bar{Xi} productions;Centrality Class;K/#pi Ratio");
    ...
    // Add legend
    TLegend* leg = new TLegend(0.7, 0.8, 0.9, 0.9);
    leg->AddEntry(hKPlus, "K+/#pi-", "lep");
    leg->AddEntry(hKMinus, "K-/#pi+", "lep");
    leg->Draw();
