...
void drawRatiosXiXibar() {
    ...
    // Open the ROOT file
    TFile* yieldsFile = new TFile("k2pi_xixibar_yields01.root", "READ");
    if (!yieldsFile || yieldsFile->IsZombie()) {
        std::cerr << "Error: Could not open k2pi_xixibar_yields01.root" << std::endl;
        return;
    }
...
for (int i = 1; i <= hPiMinusFIST->GetNbinsX(); ++i)
    ...
    hKPlusFIST->SetTitle("K^{+/-}/(#pi^{+} + #pi^{-}) ratio within #Xi and #bar{#Xi} |#Delta#eta| < 0.2;Centrality Class;K/#pi Ratio");
