...
/*hKPlus5->Divide(hPiMinus5);
  ...
  hKFromXi5->SetTitle("(K^{+}+K^{-})/(#pi^{+}+#pi^{-}) yields within #Xi-#bar{#Xi} productions |#Delta#eta| < 1;Centrality Class;Yield Ratio");
...
void drawRatiosXiXibar() {
    ...
    hKPlus5->Divide(hPiMinus5);
    hKPlus5->SetTitle("K^{+/-}/(#pi^{+} + #pi^{-}) ratio within #Xi and #bar{#Xi} |#Delta#eta| < 0.2 (PYTHIA);Centrality Class;Yield Ratio");
    hKPlus5->SetLineColor(kRed);
    hKPlus5->SetMarkerColor(kRed);
    hKPlus5->SetMarkerStyle(20);
    hKPlus5->SetMinimum(0.001);
