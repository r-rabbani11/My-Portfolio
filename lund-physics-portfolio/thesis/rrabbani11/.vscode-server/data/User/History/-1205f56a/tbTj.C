...
void analyzexixibar005(const std::string& pythiaFiles = "../pythiarun") {
    ...
    for (Long64_t i = 0; i < nEntries; i++) {
        ...
        for (auto &trk : evt->tracks) {
            ...
            if (abs(trk.eta) > 1) continue;
            // Do the pions
            if (trk.pid == 211 && abs(trk.eta - etaXi) < 0.05)
                ++nPiPlus; // Only count pi+ within 0.2 of the Xi.
            if (trk.pid == -211 && abs(trk.eta - etaXiBar) < 0.05)
                ++nPiMinus; // Only count pi- within 0.2 of the XiBar.
            // Do the kaons
...
void analyzexixibar002(const std::string& pythiaFiles = "../pythiarun") {
    ...
    for (Long64_t i = 0; i < nEntries; i++) {
        ...
        for (auto &trk : evt->tracks) {
            ...
            if (abs(trk.eta) > 1) continue;
            // Do the pions
            if (trk.pid == 211 && abs(trk.eta - etaXi) < 0.02)
                ++nPiPlus; // Only count pi+ within 0.2 of the Xi.
            if (trk.pid == -211 && abs(trk.eta - etaXiBar) < 0.02)
                ++nPiMinus; // Only count pi- within 0.2 of the XiBar.
            // Do the kaons
...
// Print bin values and error bars
  ...
  leg->AddEntry(k_sim, "K/#pi (PYTHIA, #times2)", "ep");
  leg->AddEntry(L_sim, "#Lambda/#pi (PYTHIA, #times2)", "ep");
  leg->AddEntry(X_sim, "#Xi/#pi (PYTHIA, #times6)", "ep");
  leg->AddEntry(O_sim, "#Omega/#pi (PYTHIA, #times16)", "ep");
  leg->AddEntry(k_fist, "K/#pi (FIST, #times2)", "ep");
  leg->AddEntry(L_fist, "#Lambda/#pi (FIST, #times2)", "ep");
  leg->AddEntry(X_fist, "#Xi/#pi (FIST, #times6)", "ep");
  leg->AddEntry(O_fist, "#Omega/#pi (FIST, #times16)", "ep");
