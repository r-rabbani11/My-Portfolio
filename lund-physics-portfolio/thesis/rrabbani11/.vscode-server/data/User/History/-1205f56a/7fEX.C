...
void analyzexixibarplusminus(const std::string& pythiaFiles = "../pythiarun") {
    // Open the calibration file
    TFile calibFile("calibration.root", "READ");
    TH1D* hCalib = calibFile.Get<TH1D>("hCalib");
    auto percentiles = findPercentiles(hCalib);
    ...
    // Save results to a ROOT file
    TFile outFile("k2pi_xixibar_yieldsplusminus.root", "RECREATE");
    hKPlusCent.Write();
    hPiPlusCent.Write();
    hKMinusCent.Write();
    hPiMinusCent.Write();
...
void drawRatiosXiXibarplusminus() {
    gStyle->SetOptStat(0);
    gStyle->SetTitleFontSize(0.04);

    // Open the ROOT file
    TFile* yieldsFile = new TFile("k2pi_xixibar_yieldsplusminus.root", "READ");
    if (!yieldsFile || yieldsFile->IsZombie()) {
        std::cerr << "Error: Could not open k2pi_xixibar_yieldsplusminus.root" << std::endl;
        return;
    }
    ...
    hKPlus5->Divide(hPiPlus5);
    hKPlus5->SetTitle("K^{+/-}/#pi^{+/-} ratio near #Xi and #bar{#Xi} |#Delta#eta| < 0.2 (PYTHIA);Centrality Class;Yield Ratio");
    ...
    hKMinus5->Divide(hPiMinus5);
    hKMinus5->SetLineColor(kBlue);
    hKMinus5->SetMarkerColor(kBlue);
    hKMinus5->SetMarkerStyle(21);

    // Draw
    TCanvas* c1 = new TCanvas("c1", "K/pi ratio near #Xi and #bar[#Xi] |#Delta#eta| < 0.1", 1700, 1000);
    c1->SetLogy();
    c1->SetGrid();
    hKPlus5->Draw("E1");
    ...
    leg->AddEntry(hKPlus5, "K^{+}/#pi^{+}", "lep");
    leg->AddEntry(hKMinus5, "K^{-}/#pi^{-}", "lep");
    leg->Draw();

    c1->Update();
    c1->SaveAs("outputxixibarplusminus.png");
...
void analyzexixibarFIST(const std::string& fistDirectory = "/home/rrabbani11/fist_data") {
    ...
    for (Long64_t i = 0; i < nEntries; i++) {
        ...
        for (int j = 0; j < trackArray->GetEntriesFast(); ++j) {
            ...
            if (fabs(track->fEta) > 1) continue;
            // Pi+
            if (track->fPDG == 211 && fabs(track->fEta - etaXi) < 0.2)
                ++nPiPlus;
            // Pi-
            if (track->fPDG == -211 && fabs(track->fEta - etaXiBar) < 0.2)
                ++nPiMinus;
            // K+
    ...
    // Save results to a ROOT file
    TFile outFile("k2pi_xixibar_yieldsplusminusFIST.root", "RECREATE");
...
for (int i = 1; i <= hPiFromXiFIST->GetNbinsX(); ++i)
    ...
    // Combine every two bins into one (5 bins total)
    int nOldBins = hKFromXi->GetNbinsX();
    int nNewBins = nOldBins / 2;
    double xlow = hKFromXi->GetXaxis()->GetXmin();
    double xup = hKFromXi->GetXaxis()->GetXmax();
    TH1D* hKFromXi5 = new TH1D("hKFromXi5", "K+ (5 bins)", nNewBins, xlow, xup);
    TH1D* hPiFromXi5 = new TH1D("hPiFromXi5", "Pi+ (5 bins)", nNewBins, xlow, xup);
    TH1D* hKFromXiFIST5 = new TH1D("hKFromXiFIST5", "K+ (5 bins)", nNewBins, xlow, xup);
    TH1D* hPiFromXiFIST5 = new TH1D("hPiFromXiFIST5", "Pi+ (5 bins)", nNewBins, xlow, xup);
    for (int i = 1; i <= nNewBins; ++i) {
        int bin1 = 2 * i - 1;
        int bin2 = 2 * i;
        // Sum contents
        double sumKFromXi = hKFromXi->GetBinContent(bin1) + hKFromXi->GetBinContent(bin2);
        double sumPiFromXi = hPiFromXi->GetBinContent(bin1) + hPiFromXi->GetBinContent(bin2);
        
        double sumKFromXiFIST = hKFromXiFIST->GetBinContent(bin1) + hKFromXiFIST->GetBinContent(bin2);
        double sumPiFromXiFIST = hPiFromXiFIST->GetBinContent(bin1) + hPiFromXiFIST->GetBinContent(bin2);
        // Sum errors in quadrature
        double errKFromXi = std::sqrt(std::pow(hKFromXi->GetBinError(bin1), 2) + std::pow(hKFromXi->GetBinError(bin2), 2));
        double errPiFromXi = std::sqrt(std::pow(hPiFromXi->GetBinError(bin1), 2) + std::pow(hPiFromXi->GetBinError(bin2), 2));
        double errKFromXiFIST = std::sqrt(std::pow(hKFromXiFIST->GetBinError(bin1), 2) + std::pow(hKFromXiFIST->GetBinError(bin2), 2));
        double errPiFromXiFIST = std::sqrt(std::pow(hPiFromXiFIST->GetBinError(bin1), 2) + std::pow(hPiFromXiFIST->GetBinError(bin2), 2));
        // Set new bin content and error
        hKFromXi5->SetBinContent(i, sumKFromXi);
        hKFromXi5->SetBinError(i, errKFromXi);
        hPiFromXi5->SetBinContent(i, sumPiFromXi);
        hPiFromXi5->SetBinError(i, errPiFromXi);
        hKFromXiFIST5->SetBinContent(i, sumKFromXiFIST);
        hKFromXiFIST5->SetBinError(i, errKFromXiFIST);
        hPiFromXiFIST5->SetBinContent(i, sumPiFromXiFIST);
        hPiFromXiFIST5->SetBinError(i, errPiFromXiFIST);
    }

    // Now use the new 5-bin histograms for your ratios
    if (hPiFromXi5->Integral() == 0 || hPiFromXiFIST5->Integral() == 0) {
        std::cerr << "Error: Denominator histogram has zero integral, cannot divide." << std::endl;
        yieldsFile->Close();
        return;
    }
...
/*hKPlus5->Divide(hPiMinus5);
    hKPlus5->SetTitle("K^{+/-}/(#pi^{+} + #pi^{-}) ratio within #Xi and #bar{#Xi} |#Delta#eta < 0.1|;Centrality Class;Yield Ratio");
    hKPlus5->SetLineColor(kRed);
    hKPlus5->SetMarkerColor(kRed);
    hKPlus5->SetMarkerStyle(20);
    hKPlus5->SetMinimum(0.001);
    ...
    // Draw
    TCanvas* c1 = new TCanvas("c1", "K/pi ratio within #Xi and #bar[#Xi] |#Delta#eta < 0.1|", 1700, 1000);
    c1->SetLogy();
    c1->SetGrid();
    hKPlus5->Draw("E1");
    hKMinus5->Draw("E1 SAME");
  ...
  hKFromXi5->Scale(2.);
  hKFromXi5->Divide(hPiFromXi5);
  hKFromXi5->SetTitle("(K^{+}+K^{-})/(#pi^{+}+#pi^{-}) yields near #Xi-#bar{#Xi} productions |#Delta#eta| < 1;Centrality Class;Yield Ratio");
  hKFromXi5->SetLineColor(kRed);
  hKFromXi5->SetMarkerColor(kRed);
  hKFromXi5->SetMarkerStyle(20);
  hKFromXi5->Draw("E1");
  

  hKFromXiFIST5->Divide(hPiFromXiFIST5);
  hKFromXiFIST5->SetLineColor(kBlue);
  hKFromXiFIST5->SetMarkerColor(kBlue);
  hKFromXiFIST5->SetMarkerStyle(21);
  hKFromXiFIST5->Draw("E1 SAME");
  ...
  */
  ...
  // Add legend
  TLegend* leg = new TLegend(0.895, 0.2, 1.0, 0.9);
  leg->AddEntry(hKFromXi5, "PYTHIA", "lep");
  leg->AddEntry(hKFromXiFIST5, "Thermal-FIST", "lep");
  leg->Draw();
...
void drawRatiosXiXibar() {
    ...
    hKPlus5->Divide(hPiMinus5);
    hKPlus5->SetTitle("K^{+/-}/(#pi^{+} + #pi^{-}) ratio near #Xi and #bar{#Xi} |#Delta#eta| < 0.2 (PYTHIA);Centrality Class;Yield Ratio");
    hKPlus5->SetLineColor(kRed);
    hKPlus5->SetMarkerColor(kRed);
    hKPlus5->SetMarkerStyle(20);
    hKPlus5->SetMinimum(0.001);
    ...
    // Draw
    TCanvas* c1 = new TCanvas("c1", "K/pi ratio near #Xi and #bar[#Xi] |#Delta#eta| < 0.2", 1700, 1000);
    c1->SetLogy();
    c1->SetGrid();
    hKPlus5->Draw("E1");
    hKMinus5->Draw("E1 SAME");
...
void drawRatiosXiXibarFIST() {
    ...
    // Open the ROOT file
    TFile* yieldsFile = new TFile("k2pi_xixibar_yieldsplusminusFIST.root", "READ");
    if (!yieldsFile || yieldsFile->IsZombie()) {
        std::cerr << "Error: Could not open k2pi_xixibar_yieldsplusminusFIST.root" << std::endl;
        return;
    }
...
for (int i = 1; i <= hPiMinusFIST->GetNbinsX(); ++i)
    ...
    // Combine every two bins into one (5 bins total)
    int nOldBins = hKPlusFIST->GetNbinsX();
    int nNewBins = nOldBins / 2;
    double xlow = hKPlusFIST->GetXaxis()->GetXmin();
    double xup = hKPlusFIST->GetXaxis()->GetXmax();
    TH1D* hKPlus5 = new TH1D("hKPlus5", "K+ (5 bins)", nNewBins, xlow, xup);
    TH1D* hPiPlus5 = new TH1D("hPiPlus5", "Pi+ (5 bins)", nNewBins, xlow, xup);
    TH1D* hKMinus5 = new TH1D("hKMinus5", "K- (5 bins)", nNewBins, xlow, xup);
    TH1D* hPiMinus5 = new TH1D("hPiMinus5", "Pi- (5 bins)", nNewBins, xlow, xup);
    ...
    for (int i = 1; i <= nNewBins; ++i) {
        ...
        int bin2 = 2 * i;
        // Sum contents
        double sumKPlus = hKPlusFIST->GetBinContent(bin1) + hKPlusFIST->GetBinContent(bin2);
        double sumPiPlus = hPiPlusFIST->GetBinContent(bin1) + hPiPlusFIST->GetBinContent(bin2);
        double sumKMinus = hKMinusFIST->GetBinContent(bin1) + hKMinusFIST->GetBinContent(bin2);
        double sumPiMinus = hPiMinusFIST->GetBinContent(bin1) + hPiMinusFIST->GetBinContent(bin2);
        // Sum errors in quadrature
        double errKPlus = std::sqrt(std::pow(hKPlusFIST->GetBinError(bin1), 2) + std::pow(hKPlusFIST->GetBinError(bin2), 2));
        double errPiPlus = std::sqrt(std::pow(hPiPlusFIST->GetBinError(bin1), 2) + std::pow(hPiPlusFIST->GetBinError(bin2), 2));
        double errKMinus = std::sqrt(std::pow(hKMinusFIST->GetBinError(bin1), 2) + std::pow(hKMinusFIST->GetBinError(bin2), 2));
        double errPiMinus = std::sqrt(std::pow(hPiMinusFIST->GetBinError(bin1), 2) + std::pow(hPiMinusFIST->GetBinError(bin2), 2));
        // Set new bin content and error
        hKPlus5->SetBinContent(i, sumKPlus);
        hKPlus5->SetBinError(i, errKPlus);
        hPiPlus5->SetBinContent(i, sumPiPlus);
    ...
    // Create a canvas
    TCanvas* c1 = new TCanvas("c1", "K/pi ratio near Xi-Xi-bar FIST", 1700, 1000);
    c1->SetLogy();
    c1->SetGrid();
    ...
    hKPlus5->Scale(1.);
    hKPlus5->Divide(hPiPlusFIST);
    hKPlus5->SetTitle("K^{+/-}/#pi^{+/-} ratio near #Xi and #bar{#Xi} |#Delta#eta| < 0.2 (Thermal-FIST);Centrality Class;K/#pi Ratio");
    hKPlus5->SetLineColor(kRed);
    hKPlus5->SetMarkerColor(kRed);
    hKPlus5->SetMarkerStyle(20);
    hKPlus5->SetMinimum(0.001); // Set axis range
    hKPlus5->SetMaximum(0.5);
    hKPlus5->Draw("E1");

    hKMinus5->Divide(hPiMinusFIST);
    hKMinus5->SetLineColor(kBlue);
    hKMinus5->SetMarkerColor(kBlue);
    hKMinus5->SetMarkerStyle(21);
    hKMinus5->Draw("E1 SAME");
