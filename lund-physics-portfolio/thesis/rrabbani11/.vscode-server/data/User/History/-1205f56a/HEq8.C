...
for (int i = 1; i <= hPiMinusFIST->GetNbinsX(); ++i)
    ...
    // Create a canvas
    TCanvas* c1 = new TCanvas("c1", "K/pi ratio within Xi-Xi-bar FIST", 1700, 1000);
    c1->SetLogy();
    c1->SetGrid();
    ...
    hKPlusFIST->SetTitle("K^{+/-}/#pi^{+/-} ratio within #Xi and #bar{#Xi} |#Delta#eta| < 0.2 (Thermal-FIST);Centrality Class;K/#pi Ratio");
