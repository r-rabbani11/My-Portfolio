...
for (int i = 1; i <= hPiMinusFIST->GetNbinsX(); ++i)
    ...
    hKPlusFIST->SetTitle("K^{+/-}/(#pi^{+} + #pi^{-}) ratio within #Xi and #bar{#Xi} |#Delta#eta| < 0.2 (Thermal-FIST);Centrality Class;K/#pi Ratio");
    ...
    // Add legend
    TLegend* leg = new TLegend(0.895, 0.2, 1.0, 0.9);
    leg->AddEntry(hKPlusFIST, "K^{+}/(#pi^{+} + #pi^{-})", "lep");
    leg->AddEntry(hKMinusFIST, "K^{-}/(#pi^{+} + #pi^{-})", "lep");
    leg->Draw();
