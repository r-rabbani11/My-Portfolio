...
void analyzexixibarFIST(const std::string& fistDirectory = "/home/rrabbani11/fist_data") {
    ...
    // Save results to a ROOT file
    TFile outFile("k2pi_xixibar_yieldsfinalplusminusFIST.root", "RECREATE");
...
/*hKPlus5->Divide(hPiMinus5);
  ...
  hKFromXi5->SetTitle("(K^{+}+K^{-})/(#pi^{+}+#pi^{-}) yields near #Xi-#bar{#Xi} productions |#eta| < 1;Centrality Class;Yield Ratio");
...
void drawRatiosXiXibarFIST() {
    ...
    // Open the ROOT file
    TFile* yieldsFile = new TFile("k2pi_xixibar_yieldsplusminusfinalFIST.root", "READ");
    if (!yieldsFile || yieldsFile->IsZombie()) {
        std::cerr << "Error: Could not open k2pi_xixibar_yieldsplusminusfinalFIST.root" << std::endl;
        return;
    }
...
for (int i = 1; i <= hPiMinusFIST->GetNbinsX(); ++i)
    ...
    hKPlus5->Scale(1.);
    hKPlus5->Divide(hPiPlus5);
    hKPlus5->SetTitle("K^{+/-}/(#pi^{+}+#pi^{-}) ratio near #Xi and #bar{#Xi} |#Delta#eta| < 0.2 (Thermal-FIST);Centrality Class;K/#pi Ratio");
    hKPlus5->SetLineColor(kRed);
    hKPlus5->SetMarkerColor(kRed);
    hKPlus5->SetMarkerStyle(20);
