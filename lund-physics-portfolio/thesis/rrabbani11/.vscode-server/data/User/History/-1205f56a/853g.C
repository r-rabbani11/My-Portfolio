...
for (int i = 1; i <= hPiMinusFIST->GetNbinsX(); ++i)
    ...
    for (int i = 1; i <= nNewBins; ++i) {
        ...
        int bin2 = 2 * i;
        // Sum contents
        double sumKPlus = hKPlusFIST->GetBinContent(bin1) + hKPlus->GetBinContent(bin2);
        double sumPiPlus = hPiPlusFIST->GetBinContent(bin1) + hPiPlus->GetBinContent(bin2);
        double sumKMinus = hKMinusFIST->GetBinContent(bin1) + hKMinus->GetBinContent(bin2);
        double sumPiMinus = hPiMinusFIST->GetBinContent(bin1) + hPiMinus->GetBinContent(bin2);
        // Sum errors in quadrature
        double errKPlus = std::sqrt(std::pow(hKPlusFIST->GetBinError(bin1), 2) + std::pow(hKPlus->GetBinError(bin2), 2));
        double errPiPlus = std::sqrt(std::pow(hPiPlusFIST->GetBinError(bin1), 2) + std::pow(hPiPlus->GetBinError(bin2), 2));
        double errKMinus = std::sqrt(std::pow(hKMinusFIST->GetBinError(bin1), 2) + std::pow(hKMinus->GetBinError(bin2), 2));
        double errPiMinus = std::sqrt(std::pow(hPiMinusFIST->GetBinError(bin1), 2) + std::pow(hPiMinus->GetBinError(bin2), 2));
        // Set new bin content and error
        hKPlus5->SetBinContent(i, sumKPlus);
        hKPlus5->SetBinError(i, errKPlus);
        hPiPlus5->SetBinContent(i, sumPiPlus);
