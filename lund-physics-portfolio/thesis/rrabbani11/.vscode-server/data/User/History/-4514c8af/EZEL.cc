...
int main() {
  ...
  // Junction settings (using available parameters)
  pythia.readString("ColourReconnection:mode = 1");
  pythia.readString("ColourReconnection:allowJunctions = on");
  pythia.readString("ColourReconnection:junctionCorrection = 1.5");
  
  // Alternative strangeness enhancement (using available parameters)
  pythia.readString("StringZ:aLund = 0.68");
  pythia.readString("StringZ:bLund = 0.98");
  pythia.readString("StringZ:aExtraSQuark = 0.00");
  pythia.readString("StringZ:aExtraDiquark = 0.97");
  pythia.readString("StringPT:sigma = 0.335");
  
  // Baryon production (using available parameters)
  pythia.readString("StringFlav:probStoUD = 0.19");
  pythia.readString("StringFlav:probQQtoQ = 0.078");
  pythia.readString("StringFlav:probSQtoQQ = 0.040");
  pythia.readString("StringFlav:probQQ1toQQ0 = 0.027");
      ...
      if (abs(p.id()) == 3312) {
        ...
        for (int j = 0; j < event.size(); ++j) {
          ...
          if (p2.isFinal() && abs(p2.id()) == 321 && p2.pT() > 0.5) {
            double dEta = p.eta() - p2.eta();
            double dPhi = p.phi() - p2.phi();
