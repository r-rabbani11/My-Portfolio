...
int main() {
  ...
  int nXiJunctions, nPionJunctions;
  ...
  TH1D* hXiKK = new TH1D("hXiKK", "Xi + K + K junction candidates; Mass (GeV); Counts", 100, 0, 5);
  TH1D* hPionKK = new TH1D("hPionKK", "Pion + K + K junction candidates; Mass (GeV); Counts", 100, 0, 5);
  ...
  // Enhanced junction settings based on manual section 7.1.5 and 7.2
  pythia.readString("ColourReconnection:mode = 1"); // MPI-based model
  pythia.readString("ColourReconnection:allowJunctions = on");
  pythia.readString("ColourReconnection:junctionCorrection = 1.5"); // Enhanced for Xi production
  pythia.readString("StringFragmentation:stopMass = 1.5"); // Helps with strange baryon production
  
  // Strange enhancement for Xi production
  pythia.readString("StringFragmentation:enhancedStrangeness = on");
  pythia.readString("StringFragmentation:enhancedStrangenessFactor = 2.5");
  pythia.readString("StringZ:usePetersonC = on");
  pythia.readString("StringZ:usePetersonB = on");
  
  // Baryon production parameters
  pythia.readString("StringFragmentation:probQQtoQ = 0.10"); // Increase diquark production
  pythia.readString("StringFragmentation:probQQ1toQQ0join = 0.50,0.50,0.50,0.50"); // Helps junction formation

  // Particle decays and stability
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
    ...
    nXiJunctions = nPionJunctions = 0;
    Event& event = pythia.event;

    // First pass: count all particles
    ...
    // Second pass: identify junction candidates
    for (int i = 0; i < event.size(); ++i) {
      Particle& p = event[i];
      if (!p.isFinal() || !p.isHadron() || p.pT() < 0.5) continue;

      // Check for Xi- (3312) or Xi+ (-3312)
      if (abs(p.id()) == 3312) {
        // Look for associated kaons (K+ = 321, K- = -321)
        std::vector<int> kaonIndices;
        for (int j = 0; j < event.size(); ++j) {
          if (j == i) continue;
          Particle& p2 = event[j];
          if (p2.isFinal() && abs(p2.id()) == 321 && p2.pT() > 0.5) {
            double dR = (p.p().pPos() - p2.p().pPos()).pPos().pAbs();
            if (dR < 0.5) kaonIndices.push_back(j);
          }
        }
        if (kaonIndices.size() >= 2) {
          nXiJunctions++;
          // Reconstruct invariant mass of Xi+K+K system
          Vec4 pSum = p.p();
          for (int idx : kaonIndices) pSum += event[idx].p();
          hXiKK->Fill(pSum.mCalc());
        }
      }
      // Check for pions (211 = pi+, -211 = pi-)
      else if (abs(p.id()) == 211) {
        std::vector<int> kaonIndices;
        for (int j = 0; j < event.size(); ++j) {
          if (j == i) continue;
          Particle& p2 = event[j];
