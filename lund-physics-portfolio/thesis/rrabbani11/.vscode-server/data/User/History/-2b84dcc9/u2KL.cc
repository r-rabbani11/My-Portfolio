...
int main() {
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
    ...
    if (nPions > 0) {
      hKp->Fill(nCharged, nKaons / (double)nPions);
      hLp->Fill(nCharged, nLambdas / (double)nPions);
      hXp->Fill(nCharged, nXis / (double)nPions);
      hOp->Fill(nCharged, nOmegas / (double)nPions);
    }
