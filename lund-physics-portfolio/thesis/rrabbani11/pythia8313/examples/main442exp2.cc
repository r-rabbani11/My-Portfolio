...
//MAIN PROGRAM TO SHOW WITH MULTIPLICITY CLASS AND THE EVENTS GENERATED
//remember to check hep data and download the root file to see the results
//rivet hepforge alice 2016
...
int main() {
  ...
  TFile *outFile = new TFile("main442exp2.root", "RECREATE");
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
    ...
    for (int i = 0; i < event.size(); ++i){
      ...
      if (p.isFinal() && p.isCharged()) {
        double eta = p.eta();
        if (eta > 2.8 && eta < 5.1 && eta > -1.7 && eta << -3.8) ++forwardMult; // V0M data

        // percentile classes 1, 5, 10, 15, 20, 30, 40, 50, 70, 100
