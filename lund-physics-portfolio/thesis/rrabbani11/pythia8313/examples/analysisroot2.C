...
void drawRatiosXiXibarFIST() {
    ...
    // Open the ROOT file
    TFile* yieldsFile = new TFile("k2pi_xixibar_yieldsfinalplusminusFIST.root", "READ");
    if (!yieldsFile || yieldsFile->IsZombie()) {
        std::cerr << "Error: Could not open k2pi_xixibar_yieldsfinalplusminusFIST.root" << std::endl;
        return;
    }
